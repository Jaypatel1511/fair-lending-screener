...
| `business_or_commercial_purpose == 2` | Non-business-purpose loans only | Excludes applications where the loan is for a business or commercial purpose rather than personal homeownership. Present in 2018+ HMDA data. The Markup (2021) applied this filter. |
...
- **Recommended minimum:** 1,000+ observations for stable coefficient estimation with MSA dummies
...
Peduzzi et al. established the **10 events per variable (EPV)** rule through Monte Carlo simulation: with EPV < 10, regression coefficients are biased, variance estimates are unreliable, and confidence intervals are anti-conservative. EPV is computed as the count of **denial events** (not total observations) divided by the number of predictor variables.

At the national HMDA denial rate of approximately 10% for conventional home purchase loans and a baseline model with 9 controls (before MSA dummies), EPV = 10 requires approximately **90 denial events**, which at 10% denial rate requires approximately **900 total observations**. Including MSA dummies (typically 50–350 additional predictors for national or large-state analyses) pushes the true EPV-derived minimum substantially higher.

The default minimum of **500 total observations** is a permissive threshold that accommodates state-level and single-lender analyses while blocking analyses on the smallest samples. At 500 observations with a 10% denial rate (~50 denial events) and 9 base controls, EPV ≈ 5.5 — below Peduzzi's recommended threshold of 10. Analysts should treat results from samples below 1,000 observations with additional caution, particularly when the model includes many MSA dummies. The tool provides explicit sample size disclosure in every `DisparityResult`.

**The Markup's** minimum for reporting per-metro results was 1,000 applications — a stricter threshold chosen for publication standards and one that better satisfies the Peduzzi EPV criterion for the baseline control set.
