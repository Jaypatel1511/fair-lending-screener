...
| `business_or_commercial_purpose == 2` | Not for business/commercial purpose | HMDA code 1 = business/commercial purpose; code 2 = not for business/commercial purpose. Exempt reporters use code 1111. **Policy:** only rows explicitly coded 2 are included. NaN, Exempt (1111), and missing values are excluded — erring on the side of analyzing only clearly consumer-purpose loans. This matches The Markup (2021) Filter 9. |
...
MSA dummy variables are interpretable, transparent, and auditable — each coefficient has a direct interpretation. Hierarchical linear models (mixed-effects logit) shrink MSA-level estimates toward the grand mean, which is methodologically defensible but adds complexity that makes results harder to explain to a journalist or attorney. Multilevel modeling is deferred to v0.3.0+.
...
`adjusted_denial_disparity()` requires a `data_year` keyword argument (required, no default).

**Validation rules:**
- Must be a Python `int` (not `float`, `str`, or other type)
- Must be in the range `[2018, current calendar year]`
- Raises `InvalidDataYearError` if either condition fails

**Why 2018 is the minimum:** The expanded HMDA LAR (Dodd-Frank § 1094, implemented in 12 C.F.R. Part 1003) added `loan_to_value_ratio`, `debt_to_income_ratio`, and `property_value` fields starting with the 2018 reporting year. These are three of the five FFIEC-standard controls required for a complete adjusted disparity analysis. Data years before 2018 lack these fields and cannot support the full control set.

**Why future years are rejected:** HMDA data is published annually, typically in mid-year for the prior year. Future data years do not exist in the CFPB HMDA Data Browser and requesting them would produce no data or misleading results.

`data_year` is recorded in `DisparityResult.provenance["input_parameters"]["data_year"]` for reproducibility.

---

## Sample Size Requirements

- **Minimum default:** 500 total observations (protected class + comparison class combined)
...
Peduzzi et al. established the **10 events per variable (EPV)** rule through Monte Carlo simulation: with EPV < 10, regression coefficients are biased, variance estimates are unreliable, and confidence intervals are anti-conservative. With approximately 9 regression predictors in the baseline model (before MSA dummies) and a national HMDA denial rate of approximately 10% for conventional home purchase loans, EPV = 10 implies approximately 900 total observations. At n = 500, EPV ≈ 5.5 — below the Peduzzi target but above EPV = 1.1 at the prior default of n = 100. The 500-observation minimum is the practical floor for returning any result; analysts are encouraged to use larger samples (1,000+) for stable coefficient estimation with MSA dummies.

**The Markup's** minimum for reporting per-metro results was 1,000 applications — a stricter threshold chosen for publication standards. The 500-observation minimum in this package is a floor for returning *any* result; the tool provides explicit sample size disclosure in every `DisparityResult`.
...
| Dimension | The Markup | This Package (v0.2.0) | Impact |
|---|---|---|---|
