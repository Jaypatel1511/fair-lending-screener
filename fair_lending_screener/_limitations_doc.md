...
The tool is honest about this. Every generated report includes the complete limitations list and a result-specific disclaimer. Two disclaimer variants exist, selected automatically based on whether the adjusted disparity was statistically significant (p < 0.05 with a confidence interval that excludes 1.0).

**Statistically significant result** (`is_statistically_significant=True`):
...
**Non-significant result** (`is_statistically_significant=False`, constant: `STANDARD_DISCLAIMER_NON_SIGNIFICANT`):

> "This analysis did NOT identify a statistically significant adjusted disparity at the conventional p<0.05 threshold. Absence of statistical significance does not prove the absence of disparity — it may reflect sample size, model specification, or genuinely small disparities. This tool is a screening signal and does not constitute a finding of discrimination."

The non-significant variant is intentionally not an exoneration. Absence of statistical significance at p < 0.05 in a public-data model that lacks credit score and AUS controls does not prove that no disparity exists — it means the available data are insufficient to detect one at conventional thresholds. Federal examiners with access to full loan files, credit data, and AUS records may reach a different conclusion on the same lender.
